...
export class SafetyService {
  ...
  async getSafetyPredictions(
    ...
    try {
      ...
      let predictions: RiskPrediction[];
      ...
      if (userId) {
        ...
        await this.safetyRepository.create(alertData);
      }
   ...
   */
  private transformMLPredictions(mlPredictions: Record<string, string>): RiskPrediction[] {
  ...
  async runDiagnostics(): Promise<Record<string, unknown>> {
    ...
    try {
      if (mongoose.connection.readyState === 1) {
        (results.mongodb as Record<string, unknown>).status = 'connected';
        (results.mongodb as Record<string, unknown>).message = 'MongoDB is connected';
      } else {
        (results.mongodb as Record<string, unknown>).status = 'disconnected';
        (results.mongodb as Record<string, unknown>).message = 'MongoDB is not connected';
      }
    } catch (error_) {
      (results.mongodb as Record<string, unknown>).status = 'error';
      (results.mongodb as Record<string, unknown>).message = (error_ as Error).message;
    }
    ...
    try {
      ...
      const googleMapsResult = results.googleMaps as Record<string, unknown>;
      googleMapsResult.apiKey = `${config.googleMaps.apiKey.substring(0, 10)}...`;
      ...
      if (response.data.status === 'OK') {
        googleMapsResult.status = 'working';
        googleMapsResult.message = 'Google Maps API is accessible and working';
      } else if (response.data.status === 'REQUEST_DENIED') {
        googleMapsResult.status = 'error';
        googleMapsResult.message = 'API key invalid or API not enabled';
      } else if (response.data.status === 'OVER_QUERY_LIMIT') {
        googleMapsResult.status = 'error';
        googleMapsResult.message = 'API quota exceeded';
      } else {
        googleMapsResult.status = 'error';
        googleMapsResult.message = `API returned status: ${response.data.status}`;
      }
    } catch (error_: unknown) {
      const error = error_ as { code?: string; message?: string } | null;
      const googleMapsResult = results.googleMaps as Record<string, unknown>;
      googleMapsResult.status = 'error';
      if (error?.code === 'ENOTFOUND' || error?.code === 'EAI_AGAIN') {
        googleMapsResult.message =
          'Network error: Cannot reach Google Maps API (DNS resolution failed)';
      } else if (error?.code === 'ETIMEDOUT') {
        googleMapsResult.message = 'Request timeout - network may be slow or blocking API';
      } else {
        googleMapsResult.message = error?.message || 'Unknown error';
      }
    }
    // Test 3: ML Service
    try {
      ...
      const mlServiceResult = results.mlService as Record<string, unknown>;

      if (response.status === 200) {
        mlServiceResult.status = 'working';
        mlServiceResult.message = 'ML Safety service is running';
      } else {
        mlServiceResult.status = 'error';
        mlServiceResult.message = `Service returned status: ${response.status}`;
      }
    } catch (error_: unknown) {
      const error = error_ as { code?: string; message?: string } | null;
      const mlServiceResult = results.mlService as Record<string, unknown>;
      ...
      if (error?.code === 'ECONNREFUSED') {
        mlServiceResult.message = 'ML service not running (connection refused)';
      } else if (error?.code === 'ETIMEDOUT') {
        mlServiceResult.message = 'ML service timeout';
      } else {
        mlServiceResult.message = error?.message || 'Unknown error';
      }
    }
