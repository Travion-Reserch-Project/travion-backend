...
router.post('/register', authenticate as any, apiLimiter, [
...
router.put('/location', authenticate as any, apiLimiter, [
  body('latitude').isFloat({ min: -90, max: 90 }).withMessage('Invalid latitude'),
  body('longitude').isFloat({ min: -180, max: 180 }).withMessage('Invalid longitude'),
], pushNotificationController.updateLocation);

// DELETE /api/v1/push-notifications/unregister
router.delete('/unregister', authenticate as any, apiLimiter, pushNotificationController.unregisterToken);
